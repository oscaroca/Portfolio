export function createTiles() {
  ...
  let onCombineRequest = null;
  let onDropOnCanvas = null;
  let onRemoveTile = null;
  let drag = null;
  ...
  function attachSidebarDrag(el, tileData) {
    el.addEventListener('pointerdown', (e) => {
      ...
      el.setPointerCapture(e.pointerId);
      drag = { el: clone, pointerEl: el, tileData, fromSidebar: true, offsetX: w / 2, offsetY: h / 2 };
    });
  ...
  function onPointerMoveRaw(e) {
    ...
    highlightTarget(getTileUnder(e.clientX, e.clientY, drag.el));
  }
  ...
  function onPointerUpRaw(e) {
    ...
    if (target) {
      if (drag.fromSidebar) drag.el.remove();
      if (onCombineRequest) onCombineRequest(drag.tileData, target);
    } else {
      const canvasRect = canvasEl.getBoundingClientRect();
      const sidebarEl = document.getElementById('ijk-sidebar');
      ...
      if (overSidebar && !drag.fromSidebar) {
        ...
        if (onRemoveTile) onRemoveTile(drag.tileData.instanceId);
        drag.el.remove();
        ...
        if (drag.fromSidebar) {
          ...
          if (onDropOnCanvas) onDropOnCanvas(drag.tileData, instanceId, x, y);
        } else {
          if (onDropOnCanvas) onDropOnCanvas(drag.tileData, drag.tileData.instanceId, x, y);
        }
      }
    }
  ...
  return {
    ...
    makeTileId,
    makeInstanceId,
    clearCanvas() { canvasEl.querySelectorAll('.tile').forEach(el => el.remove()); },
    clearSidebar() { sidebarTilesEl.innerHTML = ''; },
  };
}
