...
function romajiToHiragana(romaji) {
  ...
  let i = 0;
  while (i < r.length) {
    if (i + 1 < r.length && r[i] === r[i + 1] && r[i] !== 'n' && /[bcdfghjklmnpqrstvwxyz]/.test(r[i])) {
      result += 'っ';
      i++;
      continue;
    }
...
async function searchJisho(keyword) {
  try {
    const res = await fetch(`https://jisho.org/api/v1/search/words?keyword=${encodeURIComponent(keyword)}`);
    if (!res.ok) return [];
...
function extractByReading(entries, kana) {
  ...
  for (const entry of entries) {
    for (const j of entry.japanese) {
      if (j.reading === kana) {
        results.push({ kanji: j.word || kana, readingKana: j.reading, meaning: entry.senses[0].english_definitions.slice(0, 3).join(', ') });
        break;
      }
    }
  }
  ...
  for (const entry of entries) {
    for (const j of entry.japanese) {
      if (j.word === kanjiStr && j.reading) {
        results.push({ kanji: j.word, readingKana: j.reading, meaning: entry.senses[0].english_definitions.slice(0, 3).join(', ') });
        break;
      }
    }
  }
...
export const dictionary = {
  async lookup(kana, { kanaRev = null, kanji = null, kanjiRev = null } = {}) {
    ...
    unique.forEach((q, i) => {
      ...
      if (isKanjiQuery) extractByKanji(fetched[i], q).forEach(add);
      else extractByReading(fetched[i], q).forEach(add);
    });
